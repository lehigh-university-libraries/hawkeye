...
with the appropriate `Content-Type`. PDFs and multiframe images are split
locally in Docker, then uploaded one page at a time as `image/png`. The endpoint
...
and a final count go to stderr. `--include-text` retains OCR text in the report;
it is off by default because transcripts can duplicate the sensitive material.
Filenames may also be sensitive. Protect reports and endpoint logs accordingly.
