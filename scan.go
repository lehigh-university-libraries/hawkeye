...
func newCommand() *cobra.Command {
	...
	f.BoolVar(&o.includeText, "include-text", false, "Include potentially sensitive OCR text in report")
...
func run(ctx context.Context, path string, o options, stdout, stderr io.Writer) error {
	...
	for i, file := range files {
		...
		fmt.Fprintf(stderr, "Document %d/%d\n", i+1, len(files))
		count, err := pageCount(ctx, file, o)
		if err != nil {
			if err := write(result{File: file, Status: "error", OCRModel: o.model, Findings: []finding{}, Error: "page enumeration failed; document requires review"}); err != nil {
				return err
			}
			continue
		}
	...
	fmt.Fprintf(stderr, "%d pages; %d flagged for review; %d processing errors\n", pages, reviewed, failed)
	if err := ctx.Err(); err != nil {
		return err
	}
	if failed > 0 {
...
func scanPage(ctx context.Context, file string, page, count int, o options) result {
	...
	if o.analysisModel != "" {
		...
		if err != nil {
			r.Error = "assessment request failed; page requires review"
			return r
		}
